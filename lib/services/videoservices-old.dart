...
//     final argumentsDebug = '-y $inputs -filter_complex "$filterComplex" -map "[v]" -map "[a]" -r 30 -preset ultrafast $outputPath';
//
//     // Execute FFmpeg command
//     final session = await FFmpegKit.executeAsync(
//       kDebugMode ? argumentsDebug : arguments,
//       (sessionComplete) {
...
//       final filterComplex = 'concat=n=${videoPaths.length}:v=1:a=1 [v] [a]';
//
//       // Modified command to handle HEVC input and use copy when possible
//       final arguments = '''
//     -y $inputs
//     -filter_complex "$filterComplex"
//     -map "[v]"
...
//               // Try alternative command with re-encoding
//               final alternativeArguments = '''
//                 -y $inputs
//                 -filter_complex "$filterComplex"
//                 -map "[v]"
//                 -map "[a]"
...
//                   .trim();
//
//               final retrySession = await FFmpegKit.execute(alternativeArguments);
//               final retryReturnCode = await retrySession.getReturnCode();
//
//               if (!ReturnCode.isSuccess(retryReturnCode)) {
//                 final logs = await retrySession.getLogs();
//                 final errorMessage = logs.map((log) => log.getMessage()).join('\n');
//                 completer.completeError('FFmpeg error (retry): $errorMessage');
//               } else {
//                 onProgress(1.0);
//                 onMergeComplete(outputPath);
//                 completer.complete();
...
//       }
//
//       // Ensure videos have silent audio (if required)
//       final tempList = await checkAndAddSilentAudio(videoPaths);
//       videoPaths = tempList;
//
...
//
//       // Build FFmpeg command using the text file
//       final arguments = '-y -f concat -safe 0 -i "${listFile.path}" -c copy "$outputPath"';
//
//       // Create a completer for async execution
//       final completer = Completer<void>();
//
//       // Store session to ensure cleanup
//       final session = await FFmpegKit.executeAsync(
//         arguments,
//         (sessionComplete) async {
//           final returnCode = await sessionComplete.getReturnCode();
...
//       await completer.future.timeout(
//         Duration(seconds: 30 * videoPaths.length),
//         onTimeout: () {
//           throw Exception('Video merge operation timed out');
//         },
//       );
...
//   }
//
//   // Future<bool> downloadVideo({required String videoUrl}) async {
...
//   //     log("fasdfasd ${allInfo['version']['release']}");
//   //     final httpClient = http.Client();
//   //     final response = await httpClient.get(Uri.parse(videoUrl));
...
//   //       await saveVideoForIOS(videoPath: file.path, name: 'video.mp4');
//   //     }
//   //     return true;
//   //   } catch (e) {
//   //     log('Failed to download video: ${e.toString()}');
//   //     return false;
//   //   }
//   // }
//
...
//   }
// /*
//   Future<File> _downloadAndTranscode(String url, {required int height, required String bitrate}) async {
//     // Download the video
//     final cachedFile = await DefaultCacheManager().getSingleFile(url);
//     final outputPath = cachedFile.path.replaceAll('.mp4', '_${height}p.mp4');
//
...
//   }*/
// }
// ////Full working code without Progress is comented just in case
//
// // class VideoServices {
// //   Future<void> mergeVideos({
// //     required List<String> videoPaths,
...
// //     final filterComplex = 'concat=n=${videoPaths.length}:v=1:a=1 [v] [a]';
// //     final arguments =
// //         '-y $inputs -filter_complex "$filterComplex" -map "[v]" -map "[a]" -r 30 $outputPath';
// //     // '-y $inputs -filter_complex "$filterComplex" -map "[v]" -map "[a]" -r 30 -vsync vfr $outputPath';
//
// //     // Execute FFmpeg command
// //     final session = await FFmpegKit.execute(arguments);
// //     final returnCode = await session.getReturnCode();
// //     if (ReturnCode.isSuccess(returnCode)) {
// //       log('Videos successfully merged');
// //     } else {
// //       final logs = await session.getAllLogs();
// //       for (var logMessage in logs) {
// //         log('Log: ${logMessage.getMessage()}');
// //       }
// //       CustomSnackbar.showSnackbar('Failed to merge videos');
// //     }
// //   }
//
// //   Future<File> saveFile(String outputPath) async {
...
// //     final file = File(outputPath);
// //     final newPath = '${dir!.path}/output.mp4';
// //     await file.copy(newPath);
// //     log('File saved at: $newPath');
// //     return File(newPath);
// //   }
//
// //   Future<String?> changeTempToMp4(String tempPath) async {
// //     try {
// //       String newPath = tempPath.replaceAll('.temp', '.avi');
// //       File file = File(tempPath);
// //       await file.rename(newPath);
// //       return newPath;
// //     } catch (e) {
// //       log('Failed to change temp to mp4: ${e.toString()}');
// //       return null;
// //     }
// //   }
//
// //   Future<List<String?>> changeTempToMp4List(List<String> tempPaths) async {
// //     return Future.wait(tempPaths.map(changeTempToMp4));
// //   }
//
// //   Future<bool> hasAudio(String videoPath) async {
...
// //         if (logs[i].getMessage().contains('Audio:')) {
// //           return true;
// //         }
// //       }
// //       return false;
...
// //   }
//
// //   Future<String> addSilentAudio(String videoPath) async {
// //     final dir = await getApplicationDocumentsDirectory();
// //     final outputPath =
// //         '${dir.path}/${DateTime.now().millisecondsSinceEpoch}_silent_audio.mp4';
// //     final command =
// //         '-i $videoPath -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -shortest $outputPath';
// //     final session = await FFmpegKit.execute(command);
// //     final returnCode = await session.getReturnCode();
// //     if (ReturnCode.isSuccess(returnCode)) {
// //       return outputPath;
// //     } else {
// //       log('Failed to add silent audio');
// //       return '';
// //     }
// //   }
//
...
// //       final isAudio = await hasAudio(videoPath);
// //       if (!isAudio) {
// //         final newPath = await addSilentAudio(videoPath);
// //         newPaths.add(newPath);
// //       } else {
// //         newPaths.add(videoPath);
// //       }
// //     }
// //     return newPaths;
// //   }
