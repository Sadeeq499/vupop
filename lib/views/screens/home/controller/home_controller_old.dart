...
class HomeScreenController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    printLogs('============Hey I am called from home');
    getUserProfile();
...
// Helper to determine if video needs transcoding
  ...
  /*void onPageChange(int index) async {
    ...
    // currentIndex.value = index;
    // await videoControllers[index - 1].value.controller.pause().then((value) async {
  ...
  void onPageChange(int index) async {
    ...
    if (isIndexOutOfRange(index)) {
      ...
      if (index + 2 >= posts.length) {
        if (hasMorePosts.isFalse && posts.length == videoControllers.length) {
...
// For example, start preloading when user is 3 videos away from the end
      ...
      printLogs("=========Preload videoControllers.length ${posts.length}");
      printLogs("=========Preload videoControllers.length ${videoControllers.length - index}");
