  ...
  /*Future<void> getAllPosts() async {
    ...
    try {
      ...
      if (response is List<PostModel>) {
        printLogs('Posts: ${response.length}');
        posts.assignAll(response);
        printLogs('==================blocked users list ${SessionService().blockedUsers}');
        posts.removeWhere(
          (element) => element.reportedBy.contains(SessionService().user?.id) || SessionService().isBlocked(element.userId.id),
        );
        posts.sort((a, b) => (b.date ?? DateTime(0)).compareTo(a.date ?? DateTime(0)));

        /// sort the posts by recent date
        // sortPostsByRecent(); //TODO uncomment these lines to sort posts by following or location
        // await sortPostsByFollowing(); //TODO causing problem in posts names mixing the things
        // await sortPostsByLocation();
  ...
  Future<void> getAllPosts({required bool isFirstTime, bool isFromOnChange = false}) async {
    ...
    try {
      ...
      if (response != null && response.posts.isNotEmpty) {
        ...
        // Sort posts by followed users and recency
        /*final followedUsers = SessionService().followingUsersIDs;
        filteredPosts.sort((a, b) {
          bool aIsFollowed = followedUsers.contains(a.userId.id);
          bool bIsFollowed = followedUsers.contains(b.userId.id);
        ...
        currentPage.value++;
        // currentPage.value++;
        // hasMorePosts.value = filteredPosts.length == 20; // Assuming 20 posts per page
        ...
        printLogs("========isFromOnChange $isFromOnChange");
        printLogs("========isFromOnChange previousValue.value ${previousValue.value}");
        printLogs("========isFromOnChange videoControllers.length ${videoControllers.length}");
        printLogs("========isFromOnChange posts.length ${posts.length}");
        if (isFromOnChange) {
          await initializeAllControllers(previousValue.value, (posts.length - previousValue.value) >= 2 ? 2 : posts.length);
...
// Helper to determine if video needs transcoding
  ...
  void disposeController(int index) {
    ...
    if (isIndexOutOfRange(index)) {
      ...
      // if (index + 1 == videoPreLoadLimit.value) {
      //   videoPreLoadLimit.value += 1;
      //   initializeAllControllers(index + 1, videoLimitForLoading.value);
      // }
      if (index + 1 >= videoControllers.length) {
        if (posts.length == videoControllers.length) {
        } else {
          // initializeFutureController(index + 1);
    ...
    // currentIndex.value = index;
    // await videoControllers[index - 1].value.pause().then((value) async {
    //   if (posts.length < videoLimitForLoading.value) {
    //     videoLimitForLoading.value = posts.length;
    //   } else {
    //     List<int> indicesToDispose = activeVideoIndices
    //         .where((i) => (i < index - 3 || i > index + 3))
    //         .toList();
    //     for (int i in indicesToDispose) {
    //       disposeController(i);
    //       printLogs('Disposing controller for index: $i');
    //     }
    //   }
    //   if (posts.length < videoPreLoadLimit.value) {
    ...
    //   //   } else {
    //   //     // initializeFutureController(index + 1);
    //   //     initializeAllControllers(index + 1, 2);
    //   //   }
    //   // }
    // });
    // videoControllers[index].value.controller.setLooping(true);
    // videoControllers[index].value.controller.setVolume(1.0);
    // await videoControllers[index].value.controller.play();
  ...
  void onPageChange(int index) async {
    ...
    if (isIndexOutOfRange(index)) {
      ...
      printLogs('======index index + 2 >= posts.length ${index + 2 >= posts.length}');
      ...
      printLogs("======posts.length cont ${posts.length}");
      printLogs("======videoControllers.length cont ${videoControllers.length}");
      // Initialize new video controllers if needed
      if (index + 1 >= videoControllers.length) {
        await initializeAllControllers(index + 1, ((posts.length - index) > 3 ? 3 : posts.length));
      }
    }
  }
  ...
  Future<void> updateViewCount({required String postId, required int index}) async {
    ...
    try {
      ...
      printLogs("=========updateviewCountException $e");
  ...
  /// fn to update the ratings
  // Future<RatingResponse?> updateRating({
  ...
  //   final resp = await PostRepo()
  //       .rateVideoPost(stars: stars, videoId: videoId, userId: userId);
  //   if (resp != null) {
  //     posts[index].averageRating = resp.averageRating;
  //     posts.refresh();
  //     return resp;
  //   } else {
  //     return null;
  ...
  bool isIndexOutOfRange(int index) {
    return index >= posts.length || index < 0;
  }
  ...
  RxInt currentViewsPage = 1.obs;
  RxList<UserDetailModel> searchBottomSheetList = <UserDetailModel>[].obs;
  Future<List<UserDetailModel>> getUsersFromPostId(String postId) async {
    viewedUsers.clear();
    isViewedUsersLoading.value = true;
    try {
      PostUserViewsDataModel? postUserViewsDataModel = await PostRepo().getUserPostViews(postId: postId, pageNum: currentViewsPage.value);
      printLogs("postUserViewsDataModel.post![0].views! ${postUserViewsDataModel?.post?[0].views?.length}");
      if (postUserViewsDataModel != null &&
          postUserViewsDataModel.message == "Posts Found" &&
          postUserViewsDataModel.post != null &&
          postUserViewsDataModel.post!.isNotEmpty) {
        if (postUserViewsDataModel.post![0].views!.isNotEmpty) {
          for (UserDetailModel user in postUserViewsDataModel.post![0].views!) {
